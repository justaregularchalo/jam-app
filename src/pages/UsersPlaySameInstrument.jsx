...
function UsersPlaySameInstrument() {
  ...
  const getUsers = async () => {
    ...
    try {
      ...
      if(index > -1){
        ...
        setUserGenre(response.data[index].genre)
      ...
      const users = response.data.filter((eachUser)=>{
        ...
        if(eachUser.genre == response.data[index].genre && eachUser._id !== tokenId){ //filtro de solo users de la misma ciudad pero que no sea el usuario online
...
export default UsersPlaySameInstrument